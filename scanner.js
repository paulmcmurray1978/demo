...
const parseData = (data, productName) => {
  ...
  document.getElementById("text").innerHTML =
    "<h3>Matches will appear here</h3>";
  data.forEach((row, index) => {
    ...
    if (index < 10 && needs.length > 1) {
      ...
      let question =
        "In each of these lines (seperated by a / ) of things a foodbank needs " +
        needs.join(" / ") +
        ". " +
        " Does this product name fit strongly into any of them, and if it does tell me its name from that list above: " +
        items[0];
      ...
      let response = askBedrockAQuestion(question)
        .then((result) => {
          ...
          let somethingNeeded = "no";
          localStorage.setItem("needed", "no");
          ...
          if (
            ...
            !result.toLowerCase().includes("food")
          ) {
            somethingNeeded = "yes";
            localStorage.setItem("needed", "yes");
            ...
            document.getElementById("text").innerHTML =
              ...
              ` \r\r` +
              `\r\r </div>`;
            const element = document.getElementById("item_" + row.name);
            element.scrollIntoView();
          } else {
            if (localStorage.getItem("needed") == "yes") {
            } else {
              localStorage.setItem("needed", "no");
            }
          }

          // if (somethingNeeded == "no") {
          //   alert("That item is not needed, thank you for checking");
          //   location.reload();
          // }
        })
        .then(() => {
          // if (localStorage.getItem("needed") == "no") {
          //   document.getElementById("text").innerHTML = "No matches found";
          // }
        })
